...
import type { sellerPageProps } from "./SellerPage";
...
const SellerProductCreatePage = ({ setAddClick, categoryList }: sellerPageProps) => {
    const [previewOpen, setPreviewOpen] = useState(false);
    const [previewImage, setPreviewImage] = useState('');
    const [moreDetails, setMoreDetails] = useState<boolean>(false)

    const { handleSubmit, control, formState: { errors, isSubmitting }, getValues, reset } = useForm({
        defaultValues: {
            title: "",
            slug: null,
            category: [],
    ...
    const onSubmit = async () => {
        try {
            ...
            formData.append('slug', allValues?.slug || '')
            allValues.category?.forEach((id) => {
                formData.append('category[]', id!);
            });
            formData.append('price', String(allValues.price))
            formData.append('currency', allValues.currency)
            formData.append('stock', String(allValues.stock))
            formData.append('weight', String(allValues.weight) || "")
            formData.append('dimensions', allValues.dimensions || "")
            formData.append('shippingClass', allValues.shippingClass || '')
            formData.append('isPublished', String(allValues.isPublished))
            formData.append('variants', allValues.variants || '')
            formData.append('attributes', allValues.attributes || '')
